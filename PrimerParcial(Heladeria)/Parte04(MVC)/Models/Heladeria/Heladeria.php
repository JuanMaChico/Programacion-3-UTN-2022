...
class Heladeria implements \JsonSerializable
{
    ...
    /**
     * @desc: Funcion para crear el id automaticamente.
            ...
            foreach ($arrayHeladerias as $h) {
                if ($h->_sabor == $sabor && $h->_tipo == $tipo) {
                    ...
                    if ($h->getStock() >= $stock) {
                        ...
                        $imagen = $_FILES["archivo"];
                        echo "<br>";
                        $destino = "ImagenesDeVentas/" . $sabor . "_" . $tipo . "_" . $formatMail[0] . $fechaHoy . ".png";
    ...
    public static function BajarVentaSql($id)
    {
        $objetoAccesoDato = AccesoDatosSql::ObtenerObjectoAccesoDatos();
        $consulta = $objetoAccesoDato->RetornarConsulta("UPDATE ventas SET baja = :baja WHERE id = :id");
        $consulta->bindValue(':id', $id, PDO::PARAM_INT);
    ...
    /**
     * @desc Busca una venta en la base de datos por su id.
     * @param $id, id de la venta.
     * @return array con los datos de la venta.
     */
    public static function ConsultaVentaPorId($idVenta)
    {
        $objetoAccesoDato = AccesoDatosSql::ObtenerObjectoAccesoDatos();
        $consulta = $objetoAccesoDato->RetornarConsulta("SELECT * FROM ventas WHERE id = :idVenta");
